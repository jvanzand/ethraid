...
class Giant(object):
    ...
    def make_arrays(self, a_lim = (1.9, 1e2), m_lim = (1.5, 1e2), grid_num = 100, num_points = int(3e5), t_num = 100, plot_num = 30, e = 0, tp = 0, om = 0):
        ...
        self.M_anom_list = np.random.uniform(0, 2*np.pi, num_points)[..., None]
    ...
    def astro_post(self):
        ...
        chunk_num = 1000
        for i in range(0, self.num_points, chunk_num):
            print(M_anom_progression.shape)
            M_2d = M_anom_progression[i:i+chunk_num]
            print(np.shape(M_2d))
            e_arr = e_array[i:i+chunk_num]
            T = T_anom_array[i:i+chunk_num]
            
            a = self.a_list[i:i+chunk_num]
            m = self.m_list[i:i+chunk_num]
            per = self.per_list[i:i+chunk_num]
            e = self.e_list[i:i+chunk_num]
            
            Om = self.Om_list[i:i+chunk_num]
            om = self.om_list[i:i+chunk_num]
            inc = self.i_list[i:i+chunk_num]
